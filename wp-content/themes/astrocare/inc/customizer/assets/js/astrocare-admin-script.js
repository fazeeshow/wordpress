( function( $ ){
  $( document ).ready( function(){
    $( '.astrocare-btn-get-started' ).on( 'click', function( e ) {
      e.preventDefault();
      $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
      $.post( astrocare_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
  ...
  $( document ).on( 'click', '.notice-get-started-class .notice-dismiss', function () {
    ...
    $.ajax( ajaxurl,
    {
      type: 'POST',
      data: {
        action: 'astrocare_dismissed_notice_handler',
        type: type,
      }
    } );
  } );
